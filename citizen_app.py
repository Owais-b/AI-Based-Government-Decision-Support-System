...
st.set_page_config(page_title="Citizen Complaint Portal", layout="centered")

st.title("🗣️ NYC 311 Citizen Complaint Portal")
st.markdown("**Submit your complaint** — It will be reviewed and added to the government system.")
...
with st.form("citizen_form"):
    ...
    description = st.text_area("Detailed Description of the Problem *", height=120)
...
if submitted:
    if not name or not phone or not description or not borough:
        ...
        # Historical pattern check for fake complaint detection
        ...
        # Create new complaint (matches your dataset structure)
        new_complaint = {
            "unique_key": random.randint(60000000, 99999999),
            "created_date": datetime.now().isoformat(),
            "agency": "HPD" if any(x in complaint_type for x in ["HEAT", "WATER", "LEAK"]) else 
                      "DSNY" if any(x in complaint_type for x in ["COLLECTION", "DIRTY"]) else "NYPD",
        ...
        st.success(f"✅ Complaint Submitted Successfully!\n**Complaint ID:** {new_complaint['unique_key']}")
        
        if is_suspicious:
            st.warning(f"⚠️ This complaint has been flagged for manual review.\nReason: {reason}")
        else:
            st.info("Your complaint has been added to the system and will be visible to officers immediately.")
